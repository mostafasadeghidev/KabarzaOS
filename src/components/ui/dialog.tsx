...
/**
 * ⚠️ `dismissable={false}` پیش‌فرضِ **فرم‌ها**ست و عمدی: یک کلیکِ اشتباه
 * بیرونِ مودال، فرمِ نیمه‌پرشده را بی‌هشدار دور می‌ریخت — روی «افزودن
 * پروژه» که ده‌ها فیلد دارد یعنی از نو شروع کن. بستن باید صریح باشد:
 * دکمهٔ ✕ یا «انصراف».
 *
 * ⚠️ Escape هم بسته می‌شود، به همان دلیل. برای مودال‌های صرفاً نمایشی
 * (بدونِ ورودی) `dismissable` را روشن بگذارید.
 */
function DialogContent({
  ...
  /** بستن با کلیکِ بیرون و Escape. پیش‌فرض: خاموش. */
  dismissable?: boolean
}) {
  const guard = dismissable
    ? {}
    : {
